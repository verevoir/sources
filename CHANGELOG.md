...
- **New: `@verevoir/sources/gitlab`** — fourth SourceAdapter implementation, over GitLab's REST v4 API with native `fetch` (no SDK), for gitlab.com and self-hosted instances. Every contract method is implemented: `readFile` / `listFiles` / `getRepoTree` (keyset-paginated) / `isFresh` (a `HEAD` reading `X-Gitlab-Blob-Id` — no content download) / `writeFile` / `commitFiles` (**one atomic commit** — the commits API applies every action or none; create vs update decided per file by a bounded existence probe) / `ensureBranch` / `ensureFork` / `openPullRequest` (a **merge request**) / `getDefaultBranch`. Nested groups are supported, which is why a fork's MR head is `<fork project path>:<branch>` rather than `<owner>:<branch>` — the owner alone cannot name a GitLab project. Also exports `isGitlabUrl` and `parseGitlabProjectUrl` for callers that route by URL.
- **The token boundary is the design constraint, and the adapter enforces it rather than trusting its caller.** The API base comes from the project URL's own origin, so any host the adapter talks to is a host the token is sent to. It serves **gitlab.com plus exactly the hosts in `GITLAB_HOSTS`**, **HTTPS only**, and refuses anything else _before any request_ — there is deliberately no "looks like GitLab" heuristic, because a `gitlab.*` pattern hands the token to `gitlab.com.evil.io` on a prompt-injected URL, and an `http:` URL carries it in cleartext. Every request, pagination links included, is pinned to that origin; the token travels as `Authorization: Bearer`, which fetch strips on a cross-origin redirect where a custom `PRIVATE-TOKEN` header would be forwarded.
- **The empty-ref convention holds from the start.** GitLab requires a ref, so an absent **or empty** ref resolves to the default branch — `''` never reaches GitLab as `ref=`. This is the argument 0.8.1 found treated two ways inside one adapter; the GitLab adapter treats it one way across all of them, and a test drives all four ref-taking reads with `''`.
- **GitLab semantics that differ from GitHub, and are handled:** a fork's MR belongs to its _target_ project, so a duplicate (409) is found by listing there and matching `source_project_id`; an existing fork is adopted only if GitLab records it as forked from the upstream, never on name alone, and the lookup does not filter on `owned` (which GitLab reads as Owner access, hiding the fork from a Maintainer of the fork group); fork creation is asynchronous, so `ensureFork` waits for the import (bounded; a failed import is surfaced).
- **Resilience:** 429s retry per `Retry-After` (seconds or HTTP-date), jittered and capped; every request has a timeout, surfaced as a `SourceApiError` naming the path; tree walks are bounded by pages and wall-clock time and return `truncated` past either; the default branch is remembered for a minute, so ref-less reads do not double their calls against GitLab's rate limits (`resetGitlabCaches()` forgets it).
- No new dependencies. **No migration** — additive.
