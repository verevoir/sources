...
export interface SourceAdapter {
  ...
  /** Merge an open pull request (by number, a numeric string, or its html_url).
   * `method` selects the host merge strategy (default a merge commit). Resolves
   * true when merged; throws when the host cannot merge (conflicts, checks not
   * green) or the source has no PR concept. */
  mergePullRequest(
    env: SourceEnv,
    targetUrl: string,
    pull: number | string,
